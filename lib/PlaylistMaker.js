...
class PlaylistMaker {
   ...
   */
  async makeList() {
    const dirs = await this._directoriesWithPlayableTracks(this.directory);

    if (this.randomizeDirs) {
      dirs.sort(() => CryptRNG.nextInt32);
    }
    if (this.randomizeFiles) {
      dirs.forEach(dir => {
        dir.files.sort(() => CryptRNG.nextInt32);
      });
    }
   ...
   */
  async _directoriesWithPlayableTracks(startDir) {
    ...
    const prom = new Promise((resolve, reject) => {
      fs.readdir(startDir, async(err, files) => {
        ...
        for (const file of files) {
          ...
          if (stat.isDirectory()) {
            dwf.push(...await this._directoriesWithPlayableTracks(path.join(absPath)));
